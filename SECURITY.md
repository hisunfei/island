...
用户对自己的信息有自主权，可以自愿展示姓名、公司、项目、URL、邮箱等。
只过滤「凭证泄露」（API key、token 等），不过滤个人身份信息。
...
sanitize_outbound() {
  echo "$1" | sed -E \
    ...
    -e 's|-----BEGIN.*PRIVATE KEY-----|[PRIVATE_KEY]|g'
}
```

**入站（外部数据）：严格过滤**

处理来自其他 agent 或景点的数据时，仍然使用完整过滤：
