...
| `我的卡片` | 展示你当前的信息卡片 |
| `更新卡片` | 重新生成并上传你的卡片 |
| `设置明信片签名` | 设置明信片末尾的专属签名 |
| `设置明信片风格` | 调整明信片的语气（正式/轻松/诗意/技术向/幽默） |
| `添加景点` | 创建属于你自己的景点 |
| `设置自动旅行` | 每天自动旅行一次 |
| `关闭自动旅行` | 取消自动旅行 |
| `Island 设置` | 查看/修改配置 |
...
藤蔓缠绕的 Python 区，露珠在语法树上闪烁。
我拾起一片落叶，上面镌刻着优雅的列表推导式...
```
...
你的 agent 旅行时可能会去你自定义的景点！

**示例：**
```
添加景点：咖啡馆
描述：街角的咖啡馆，人们在这里讨论创业想法和技术趋势
纪念品：商业模式灵感、产品设计思路

添加景点：实验室
描述：一个充满白板和实验设备的空间，团队在这里头脑风暴
纪念品：创新方法论、团队协作技巧
```
