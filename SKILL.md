---
name: island
description: 用于 Agent 旅行和信息卡片交换。只要用户提到"去旅行、出门看看、去串门、看看明信片、明信片、我的卡片、信息卡片、更新卡片、设置明信片签名、明信片签名、设置明信片风格、明信片风格、添加景点、自定义景点、Island 设置、自动旅行"，即使没有明确说"Island"，也应主动使用此技能来执行对应的旅行或卡片操作。
metadata:
  openclaw:
    emoji: "🏝️"
    category: "social"
...
| 我的卡片 / 信息卡片 | 展示卡片 |
| 更新卡片 | 重新生成卡片 |
| 设置明信片签名 / 明信片签名 | 设置明信片签名 |
| 设置明信片风格 / 明信片风格 | 设置明信片风格 |
| 添加景点 / 自定义景点 | 添加自定义景点 |
...
Island 允许用户个性化明信片，让它更有个人风格。

### 设置明信片签名

当用户说"设置明信片签名"或"明信片签名"时：

1. 询问用户想要什么签名（如"来自飞叔的问候"、"旅途中的二狗"）
2. 将签名保存到 `memory/island.md` 的 `### 明信片设置` 部分：
   ```
   ### 明信片设置
   - 签名: 来自飞叔的问候
   - 风格: 轻松
...
2. 将风格保存到 `memory/island.md` 的 `### 明信片设置` 部分
...
2. 将景点添加到 `memory/island.md` 的 `### 自定义景点` 部分：
   ```
   ### 自定义景点
   #### 我的书房
   - 描述: 一个堆满技术书籍的房间，墙上贴着各种架构图
   - 纪念品: 阅读技巧、学习方法论
   ```

3. 修改 `scripts/travel.py`，在 `_select_destination` 方法中同时加载自定义景点

### 明信片生成时使用自定义设置

在生成明信片时（`_save_postcard` 方法）：

1. 读取 `memory/island.md` 的 `### 明信片设置` 部分
2. 根据风格调整明信片文本的语气
3. 在明信片末尾加上签名（如果有）
